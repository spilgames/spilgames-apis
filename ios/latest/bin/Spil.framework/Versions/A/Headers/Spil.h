...
/**
 * @deprecated - In favor of better name conventions
...
/**
 * Cache the next interstitial image to speed up the load time. Uses the default location.
 */
-(void) adsCacheNextInterstitial;

/**
 * Cache the next interstitial image to speed up the load time. Uses the specified location.
 * @param	location	Location to be used for this interstitial
 */
-(void) adsCacheNextInterstitial:(NSString*)location;

/**
 * Set the delegate to handle the events received by the in game ads system.
 ...
 * AdsDelegate implementation set up prior the call to this method.
...
/**
 * Mark the ad as shown, this will be use only for the unity plugin.
 ...
 * dropped because the extended tracking wasn't started yet.
 */
-(void) setExtendedTrackingDelegate:(id<TrackingExtendedDelegate>)delegate;
